...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 11 (Expanded Data + Slang + Lexicon)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v11"),
        help="Folder output artefak model hybrid v11",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def train_hybrid_v11(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V11")
    print("=" * 70)
    ...
    model_path = output_dir / "sentiment_model_v11.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v11.joblib"
    hybrid_path = output_dir / "hybrid_rules_v11.json"
    metadata_path = output_dir / "metadata_v11.json"
    ...
    print("=" * 70)
    print(f"Simpan model v11        : {model_path.resolve()}")
    print(f"Simpan vectorizer v11   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v11        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v11     : {metadata_path.resolve()}")
    print("=" * 70)
