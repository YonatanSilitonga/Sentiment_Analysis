...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 5 (rebalanced train + custom class weight)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v5"),
        help="Folder output artefak model hybrid v5",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def rebalance_train_dataframe(train_df: pd.DataFrame, random_state: int) -> pd.DataFrame:
    # Target distribusi train yang lebih seimbang agar kelas positif/netral tidak kalah jauh.
    target_ratio = {
        "negatif": 0.40,
        "positif": 0.35,
        "netral": 0.25,
...
def tune_model_with_class_weights(
    ...
    candidates: list[tuple[str, str | dict[str, float]]] = [
        ("balanced", "balanced"),
        ("custom_v1", {"negatif": 0.9, "netral": 1.1, "positif": 1.3}),
        ("custom_v2", {"negatif": 0.85, "netral": 1.15, "positif": 1.4}),
    ]
                ...
                for strong_prob in strong_prob_candidates:
                    config = {
                        **default_hybrid_config(),
                        ...
                        "positive_mid_confidence_threshold": 0.20,
...
def train_hybrid_v5(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    hybrid_config = {
        **hybrid_config,
        "positive_override_enabled": True,
        "positive_label": "positif",
        "positive_keyword_score_threshold": int(hybrid_config.get("positive_keyword_score_threshold", 3)),
        "positive_mid_confidence_threshold": float(hybrid_config.get("positive_mid_confidence_threshold", 0.20)),
        "positive_block_negative_prob": float(hybrid_config.get("positive_block_negative_prob", 0.65)),
    }

    print("=" * 70)
    print("TRAINING MODEL HYBRID V5")
    print("=" * 70)
    ...
    print("-" * 70)
    print("BASE MODEL (rebalanced + tuned class_weight)")
    ...
    # Refit final model pada full data dengan class_weight terbaik
    selected_weight_name = str(base_metrics["selected_class_weight_name"])
    selected_weight_map: str | dict[str, float]
    if selected_weight_name == "custom_v1":
        selected_weight_map = {"negatif": 0.9, "netral": 1.1, "positif": 1.3}
    elif selected_weight_name == "custom_v2":
        selected_weight_map = {"negatif": 0.85, "netral": 1.15, "positif": 1.4}
    else:
    ...
    model_path = output_dir / "sentiment_model_v5.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v5.joblib"
    hybrid_path = output_dir / "hybrid_rules_v5.json"
    metadata_path = output_dir / "metadata_v5.json"
    ...
    metadata = {
        ...
        "version": "v5_hybrid_rebalanced_custom_weight",
        "data_path": str(data_path.resolve()),
        "model_path": str(model_path.resolve()),
        "vectorizer_path": str(vectorizer_path.resolve()),
        "hybrid_rules_path": str(hybrid_path.resolve()),
        "best_model": "logistic_regression_hybrid_v5",
        "selection_metric": "macro_f1_then_positive_recall_then_negative_recall_then_accuracy",
        ...
        "data_source": "combined_rebalanced: original (379) + new Toba (400)",
    ...
    print("=" * 70)
    print(f"Simpan model v5        : {model_path.resolve()}")
    print(f"Simpan vectorizer v5   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v5        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v5     : {metadata_path.resolve()}")
    print("=" * 70)
