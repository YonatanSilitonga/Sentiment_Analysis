...
try:
    from sentiment_model.hybrid import NEGATIVE_SIGNALS, apply_hybrid_rules, default_hybrid_config
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid (ML + negative keyword rules) versi 3"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v3"),
        help="Folder output artefak model hybrid v3",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def train_hybrid_v3(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V3")
    print("=" * 70)
    ...
    print("-" * 70)
    print("HYBRID MODEL (base + extended rules)")
    ...
    model_path = output_dir / "sentiment_model_v3.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v3.joblib"
    hybrid_path = output_dir / "hybrid_rules_v3.json"
    ...
    print("=" * 70)
    print(f"Simpan model v3     : {model_path.resolve()}")
    print(f"Simpan vectorizer v3: {vectorizer_path.resolve()}")
    print(f"Simpan rules v3     : {hybrid_path.resolve()}")
    print(f"Simpan metadata v3  : {metadata_path.resolve()}")
    print("=" * 70)
