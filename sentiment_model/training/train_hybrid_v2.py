...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid (ML + negative keyword rules) versi 2"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v2"),
        help="Folder output artefak model hybrid v2",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def train_hybrid_v2(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V2")
    print("=" * 70)
    ...
    # Refit ke seluruh data berlabel untuk artefak final v2.
    X_all_tfidf = vectorizer.fit_transform(X_text)
    base_model.fit(X_all_tfidf, y)

    output_dir.mkdir(parents=True, exist_ok=True)
    model_path = output_dir / "sentiment_model_v2.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v2.joblib"
    hybrid_path = output_dir / "hybrid_rules_v2.json"
    ...
    metadata = {
        ...
        "best_model": "logistic_regression_balanced_hybrid",
    ...
    print("=" * 70)
    print(f"Simpan model v2     : {model_path.resolve()}")
    print(f"Simpan vectorizer v2: {vectorizer_path.resolve()}")
    print(f"Simpan rules v2     : {hybrid_path.resolve()}")
    print(f"Simpan metadata v2  : {metadata_path.resolve()}")
    print("=" * 70)
