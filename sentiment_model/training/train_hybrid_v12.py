...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 12 (Optimized Hybrid Rules + Negation Scope)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path("Merged_Excel/dataset_v11_master.xlsx"),
        help="Path file dataset berlabel (.xlsx)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v12"),
        help="Folder output artefak model hybrid v12",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def tune_hybrid_config(result_df: pd.DataFrame, raw_texts: list[str], y_true: pd.Series) -> tuple[dict[str, Any], dict[str, Any]]:
    from sentiment_model.hybrid import (
        compute_negative_keyword_score,
        compute_positive_keyword_score,
        compute_neutral_keyword_score,
        compute_contrast_keyword_score,
    )
    
    print("Precomputing keyword scores for validation set...")
    precomputed_data = []
    for idx, raw_text in enumerate(raw_texts):
        text_str = str(raw_text)
        score_neg, _, _, inv_pos = compute_negative_keyword_score(text_str)
        score_pos, _, _ = compute_positive_keyword_score(text_str, extra_pos_from_negation=inv_pos)
        score_neutral, _, _ = compute_neutral_keyword_score(text_str)
        score_contrast, _, _ = compute_contrast_keyword_score(text_str)
        word_count = len(text_str.split())
        
        prob_neg = float(result_df.at[idx, "prob_negatif"])
        prob_pos = float(result_df.at[idx, "prob_positif"])
        base_label = str(result_df.at[idx, "prediksi_sentimen"])
    ...
    print("Precomputation finished. Starting grid search...")
    ...
    def evaluate_config(config):
        preds = []
        negative_label = config.get("negative_label", "negatif")
        positive_label = config.get("positive_label", "positif")
        neutral_label = config.get("neutral_label", "netral")
        
        high_threshold = float(config.get("high_confidence_threshold", 0.55))
        mid_threshold = float(config.get("mid_confidence_threshold", 0.30))
        mid_score_threshold = int(config.get("mid_keyword_score_threshold", 2))
        strong_score_threshold = int(config.get("strong_signal_score_threshold", 3))
        strong_min_prob = float(config.get("strong_signal_min_prob", 0.15))

        positive_override_enabled = bool(config.get("positive_override_enabled", False))
        positive_score_threshold = int(config.get("positive_keyword_score_threshold", 3))
        positive_mid_prob_threshold = float(config.get("positive_mid_confidence_threshold", 0.30))
        positive_block_negative_prob = float(config.get("positive_block_negative_prob", 0.65))

        neutral_override_enabled = bool(config.get("neutral_override_enabled", False))
        neutral_min_positive_score = int(config.get("neutral_min_positive_score", 2))
        neutral_max_negative_score = int(config.get("neutral_max_negative_score", 1))
        neutral_max_negative_prob = float(config.get("neutral_max_negative_prob", 0.35))
        
        for item in precomputed_data:
            word_count = item["word_count"]
            score_neg = item["score_neg"]
            score_pos = item["score_pos"]
            score_neutral = item["score_neutral"]
            ...
            # 1. Short Text Neutrality
            if word_count <= 2 and score_neg == 0 and score_pos == 0:
                final_label = neutral_label
            # 2. Strong Negation/Positive Override
            elif score_pos >= strong_score_threshold and score_neg <= 1:
    ...
    best_metrics = evaluate_config(best_config)
    ...
    neutral_override_candidates = [True, False]
    positive_override_candidates = [True, False]
            ...
            for mid_score in mid_score_candidates:
                for strong_prob in strong_prob_candidates:
                    for neutral_neg_prob in neutral_neg_prob_candidates:
                        for neutral_override in neutral_override_candidates:
                            for positive_override in positive_override_candidates:
                                config = {
                                    **default_hybrid_config(),
                                    ...
                                    "positive_override_enabled": positive_override,
                                    "positive_label": "positif",
                                    "positive_keyword_score_threshold": 3,
                                    "positive_mid_confidence_threshold": 0.22,
                                    "positive_block_negative_prob": 0.62,
                                    "neutral_override_enabled": neutral_override,
                                    "neutral_label": "netral",
                                    "neutral_min_positive_score": 2,
                                    "neutral_max_negative_score": 1,
...
def train_hybrid_v12(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    model_path = output_dir / "sentiment_model_v12.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v12.joblib"
    hybrid_path = output_dir / "hybrid_rules_v12.json"
    metadata_path = output_dir / "metadata_v12.json"
    ...
    metadata = {
        ...
        "data_source": "dataset_v11_master_13k_rows",
    ...
    print("=" * 70)
    print(f"Simpan model v12        : {model_path.resolve()}")
    print(f"Simpan vectorizer v12   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v12        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v12     : {metadata_path.resolve()}")
    print("=" * 70)
