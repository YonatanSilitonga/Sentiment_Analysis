...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 7 (same data, improved neutral handling)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path("Merged_Excel/dataset_labeled_combined_v2.xlsx"),
        help="Path file dataset berlabel (.xlsx)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v7"),
        help="Folder output artefak model hybrid v7",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def tune_model_with_class_weights(
    ...
    candidates: list[tuple[str, str | dict[str, float]]] = [
        ("balanced", "balanced"),
        ("custom_v1", {"negatif": 0.9, "netral": 1.2, "positif": 1.2}),
        ("custom_v2", {"negatif": 0.85, "netral": 1.25, "positif": 1.2}),
    ]
...
def train_hybrid_v7(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V7")
    print("=" * 70)
    ...
    if selected_weight_name == "custom_v1":
        selected_weight_map: str | dict[str, float] = {"negatif": 0.9, "netral": 1.2, "positif": 1.2}
    elif selected_weight_name == "custom_v2":
        selected_weight_map = {"negatif": 0.85, "netral": 1.25, "positif": 1.2}
    else:
    ...
    model_path = output_dir / "sentiment_model_v7.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v7.joblib"
    hybrid_path = output_dir / "hybrid_rules_v7.json"
    metadata_path = output_dir / "metadata_v7.json"
    ...
    metadata = {
        ...
        "version": "v7_hybrid_contrast_neutral_override",
        "data_path": str(data_path.resolve()),
        "model_path": str(model_path.resolve()),
        "vectorizer_path": str(vectorizer_path.resolve()),
        "hybrid_rules_path": str(hybrid_path.resolve()),
        "best_model": "logistic_regression_hybrid_v7",
        "selection_metric": "macro_f1_then_neutral_recall_then_positive_then_negative_then_accuracy",
        ...
        "data_source": "combined_v2_no_new_addition_for_v7_training",
    ...
    print("=" * 70)
    print(f"Simpan model v7        : {model_path.resolve()}")
    print(f"Simpan vectorizer v7   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v7        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v7     : {metadata_path.resolve()}")
    print("=" * 70)
