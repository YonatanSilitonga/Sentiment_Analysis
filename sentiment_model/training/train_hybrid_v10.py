...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 10 (Improved preprocessing + superlative keywords)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v10"),
        help="Folder output artefak model hybrid v10",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def train_hybrid_v10(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V10")
    print("=" * 70)
    ...
    model_path = output_dir / "sentiment_model_v10.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v10.joblib"
    hybrid_path = output_dir / "hybrid_rules_v10.json"
    metadata_path = output_dir / "metadata_v10.json"
    ...
    metadata = {
        ...
        "version": "v10_improved_preprocessing_superlative_keywords",
        "data_path": str(data_path.resolve()),
        "model_path": str(model_path.resolve()),
        "vectorizer_path": str(vectorizer_path.resolve()),
        "hybrid_rules_path": str(hybrid_path.resolve()),
        "best_model": "logistic_regression_hybrid_v10",
        ...
        "data_source": "combined_v3_with_phrase_preservation_and_soundproof",
    ...
    print("=" * 70)
    print(f"Simpan model v10        : {model_path.resolve()}")
    print(f"Simpan vectorizer v10   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v10        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v10     : {metadata_path.resolve()}")
    print("=" * 70)
