...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training model sentimen hybrid versi 8 (neutral-augmented + long-text-ready baseline)"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("sentiment_model/model_artifacts_v8"),
        help="Folder output artefak model hybrid v8",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def tune_model_with_class_weights(
    ...
    candidates: list[tuple[str, str | dict[str, float]]] = [
        ("balanced", "balanced"),
        ("custom_v1", {"negatif": 0.95, "netral": 1.15, "positif": 1.0}),
        ("custom_v2", {"negatif": 0.9, "netral": 1.2, "positif": 1.0}),
    ]
...
def train_hybrid_v8(data_path: Path, output_dir: Path, test_size: float, random_state: int) -> None:
    ...
    print("=" * 70)
    print("TRAINING MODEL HYBRID V8")
    print("=" * 70)
    ...
    if selected_weight_name == "custom_v1":
        selected_weight_map: str | dict[str, float] = {"negatif": 0.95, "netral": 1.15, "positif": 1.0}
    elif selected_weight_name == "custom_v2":
        selected_weight_map = {"negatif": 0.9, "netral": 1.2, "positif": 1.0}
    else:
    ...
    model_path = output_dir / "sentiment_model_v8.joblib"
    vectorizer_path = output_dir / "tfidf_vectorizer_v8.joblib"
    hybrid_path = output_dir / "hybrid_rules_v8.json"
    metadata_path = output_dir / "metadata_v8.json"
    ...
    metadata = {
        ...
        "data_source": "combined_v3_with_added_neutral_package",
    ...
    print("=" * 70)
    print(f"Simpan model v8        : {model_path.resolve()}")
    print(f"Simpan vectorizer v8   : {vectorizer_path.resolve()}")
    print(f"Simpan rules v8        : {hybrid_path.resolve()}")
    print(f"Simpan metadata v8     : {metadata_path.resolve()}")
    print("=" * 70)
